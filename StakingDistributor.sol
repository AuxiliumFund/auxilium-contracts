...
contract Distributor is Ownable {
    ...
    function distribute() external returns ( bool ) {
        if ( nextEpochAuxl <= uint32(block.timestamp) ) {
            nextEpochAuxl = nextEpochAuxl.add32( epochLength ); // set next epoch time
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return AUXL.totalSupply().mul( _rate ).div( 1000000 );
    }
